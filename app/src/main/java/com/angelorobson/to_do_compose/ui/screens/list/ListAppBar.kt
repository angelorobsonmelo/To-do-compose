...
import com.angelorobson.to_do_compose.components.DisplayAlertDialog
...
fun SortAction(
    ...
    IconButton(onClick = {
        ...
        DropdownMenu(
            ...
            Priority.values().slice(setOf(0, 2, 3)).forEach { action ->
                DropdownMenuItem(
                    onClick = {
                        expanded = false
                        onSortClicked(action)
                    }
                ) {
                    PriorityItem(priority = action)
                }
            }
